...
describe("PUT /api/comisiones/:id", () => {
    it("debería devolver el comision con el nroComision 238 modificado", async () => {
        const res = await request(app).put("/api/comisiones/238").send(comisionPUT);
        expect(res.statusCode).toEqual(200);
    });
});

describe("DELETE /api/comisiones/:id", () => {
    it("debería devolver la comision con el nroComision 923 borrado", async () => {
        const res = await request(app).delete("/api/comisiones/923");
        expect(res.statusCode).toEqual(200);
    });
});
